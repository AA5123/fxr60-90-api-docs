...
GET request examples are `verbose_false` and `verbose_true`. GET responses include those same names plus one sample per mode (`SIMPLE`, `INVENTORY`, `PORTAL`, `CONVEYOR`, `CUSTOM`). `verbose_true` / `INVENTORY` share the verbose response; `verbose_false` / `CUSTOM` share the configured-only response.
...
| `GET/CONVEYOR.json` | `CONVEYOR` | Response: CONVEYOR mode |
...
- No `rssiFilter` on PUT examples (optional schema field)
